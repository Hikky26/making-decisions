...
// CODE HERE
const numberArr = numbers.map(function(numElem){
  return numElem/2
})
...
const filtered = cities.filter(city => city.indexOf('i') !== -1 )
...
// CODE HERE
const totalPriceOfFoods = foods.filter(element => element.tags.includes('rice')).reduce((acc,curr) => acc + curr.price, 0)

console.log(totalPriceOfFoods)
